...
conn = sqlite3.connect("Simple_Students_Sys.db")

cursor = conn.cursor()

cursor.execute("""CREATE TABLE IF NOT EXISTS students_system (id INTEGER PRIMARY KEY AUTOINCREMENT,
               name TEXT,
               age TEXT, 
               grade TEXT)""")
conn.commit()
...
def load_students():
    cursor.execute("SELECT * FROM students_system")
    rows = cursor.fetchall()
    for row in rows:
        students_listbox.insert(tk.END, f"Name : {row[1]}"
                                        f"Age : {row[2]}"
                                        f"Grade : {row[3]}"
                                        f"──────────────────")
...
def add_student():
    ...
    if name != "" and age != "" and grade != "" :
        cursor.execute("INSERT INTO students_system (name, age, grade) VALUES (?,?,?)", (name,age,grade))
        conn.commit()
...
def delete_student():
    ...
    if name != "":
        cursor.execute("SELECT * FROM students_system WHERE name=?", (name))
