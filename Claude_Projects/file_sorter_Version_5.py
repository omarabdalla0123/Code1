...
def scan():
    ...
    others_rule["listbox"].delete(0, tk.END)
    others_rule["count"].config(text="")
    ...
    for filename in os.listdir(DOWNLOADS):
        ...
        others_rule["listbox"].insert(tk.END, "  " + filename)
        total += 1
    ...
    for r in [img_rule, others_rule]:
        n = r["listbox"].size()
        r["count"].config(text=f"{n} file{'s' if n != 1 else ''}")
...
def move_all():
    ...
    for rule in TAG_RULES + [img_rule, others_rule]:
...
# ── Notice ────────────────────────────────────────────────────────────────
notice = tk.Frame(root, bg="#FEF9C3", padx=16, pady=8)
notice.pack(fill=tk.X)
tk.Label(notice, text="Skipping: folders  |  files with (OP)",
         font=("Segoe UI", 9), bg="#FEF9C3", fg="#854D0E").pack(anchor="w")
...
make_card(cards_frame, others_rule, "Everything else")
